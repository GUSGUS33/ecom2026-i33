...
export const ProductosDinamicosBlock = React.memo(function ProductosDinamicosBlock({ data, pageUri, pageTitle, parentUri }: ProductosDinamicosBlockProps) {
  ...
  // Fallback: si no hay categoría configurada, usar el slug de la página actual
  const categorySlugToUse = productosDinamicosCategoria || (pageUri ? pageUri.replace(/^\//g, '').replace(/\/$/g, '') : null);
              ...
              {filteredProducts.map((product) => {
                ...
                return (
                  <div
                    ...
                    className="group border border-transparent hover:border-slate-200 hover:shadow-lg transition-all duration-200 p-4 rounded-sm bg-white h-full flex flex-col"
                  >
                    {/* Imagen del producto - clickeable */}
                    <Link href={`/producto/${product.slug}`}>
                      <div className="relative aspect-square overflow-hidden mb-4 bg-slate-50 rounded-sm cursor-pointer">
                        {displayImage ? (
                          <img
                            src={displayImage}
                            alt={displayAlt}
                        ...
                        {selectedVar && (
                          <div className="absolute bottom-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-full text-xs font-medium text-slate-700">
                    ...
                    <div className="flex-grow flex flex-col">
                      {/* Título clickeable */}
                      <Link href={`/producto/${product.slug}`}>
                        <h3 className="font-bold text-slate-900 text-sm mb-1 line-clamp-2 group-hover:text-blue-700 transition-colors cursor-pointer">
                          {product.name}
                        </h3>
                      </Link>
